...
- [ ] **L1.1: Database Schema**
    - [ ] Run SQL for `tasks` table in Supabase
    - [ ] Verify table existence and indices
- [ ] **L1.2: Database Package Update**
    - [ ] Add `Task` interface to `@workspace/supabase`
    - [ ] Update `toCamel`/`toSnake` to handle `tasks` table (Already done in previous step)
- [ ] **L1.3: Producer Logic (Refactoring API)**
    - [ ] Refactor `POST /generate` to insert rows into `tasks`
    - [ ] Implement batched task insertion for 1,000+ rows
    - [ ] Stop the brittle "fire-and-forget" async IIFE loop
